...
def test_overall_covid_cases_match_sum_of_total_cases_per_country():
    response = requests.get(f'{COVID_TRACKER_HOST}/api/v1/summary/latest')
    pretty_print(response.headers)
    ...
    print(search_for)
    cases_total_counted=0
    for region in search_for(tree):
        cases_total_counted += int(region.text)


    assert_that(int(total_cases)).is_lower_than(cases_total_counted)